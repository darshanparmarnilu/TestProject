...
final class DatabaseManager {
    ...
    func insertDetail(strfirstName:String,strlastName:String,stremail:String,strpass:String,strphone:String,straboutme:String,strImage:String,strDateofBirth:String,strBirthtime:String,strCountry:String,strState:String,strCity:String,strGender:String,completion:@escaping (Bool,String)->()) {
        let detail = ["firstname":strfirstName,"lastname":strlastName,"email":stremail,"password":strpass,"mobileno":strphone,"aboutme":straboutme,"image_url":strImage,"date":strDateofBirth,"time":strBirthtime,"country":strCountry,"state":strState,"city":strCity,"gender":strGender] as [String : Any]
        
        if DatabaseManager.database != nil {
            DatabaseManager.database.executeUpdate("INSERT INTO user_detail (firstname,lastname,email,password,mobileno,aboutme,image_url,date,time,country,state,city,gender) VALUES (:firstname,:lastname,:email,:password,:mobileno,:aboutme,:image_url,:date,:time,:country,:state,:city,:gender)", withParameterDictionary: detail)
    ...
    func updateData(strUserid:String,strfirstName:String,strlastName:String,strphone:String,straboutme:String,strImage:String,strDateofBirth:String,strBirthtime:String,strCountry:String,strState:String,strCity:String,strGender:String,completion:@escaping (Bool,String)->()) -> Bool {
        let updateQuery = "UPDATE user_detail SET firstname=?, lastname=?,mobileno=?,aboutme=?,image_url=?, date=?,time=?,country=?,state=?,city=?,gender=?WHERE id=?"
        self.openDataBase()
        if let database = DatabaseManager.database {
            if database.open() {
                let updateResult = database.executeUpdate(updateQuery, withArgumentsIn: [strfirstName,strlastName,strphone,straboutme,strImage,strDateofBirth,strBirthtime,strCountry,strState,strCity,strGender,strUserid])
                if !updateResult {
        ...
        if let result = DatabaseManager.database.executeQuery(selectQuery, withArgumentsIn: selectionArgs) {
            ...
            while result.next() {
                if  let u_email = result.string(forColumn: DatabaseKey.userEmailkey),
                    ...
                    let u_dateofbirth = result.string(forColumn: DatabaseKey.userDateofBirth),
                    let u_birthtime = result.string(forColumn: DatabaseKey.userBirthTime),
                    let u_country = result.string(forColumn: DatabaseKey.userCountry),
                    let u_state = result.string(forColumn: DatabaseKey.userState),
                    let u_city = result.string(forColumn: DatabaseKey.userCity),
                    let u_gender = result.string(forColumn: DatabaseKey.userGender)
                    
                {
                    
                    if u_email == email && u_password == pass{
                        
                        userDefaultsDataSave(strid: u_id, strfirstname: u_firstname, strlastname: u_lastname, stremail: u_email, strpass: u_password, strmobileNo: u_mobileno, straboutme: u_aboutme,strImage: u_image,strDateofBirth: u_dateofbirth,strBirthTime: u_birthtime,strCountry: u_country,strState: u_state,strCity: u_city,strGender: u_gender)
        ...
        if let get = DatabaseManager.database.executeQuery(selectQuery,withArgumentsIn: selectionArgs){
            while get.next(){
                if  let u_email = get.string(forColumn: DatabaseKey.userEmailkey),
                    ...
                    let u_dateofbirth = get.string(forColumn: DatabaseKey.userDateofBirth),
                    let u_birthtime = get.string(forColumn: DatabaseKey.userBirthTime),
                    let u_country = get.string(forColumn: DatabaseKey.userCountry),
                    let u_state = get.string(forColumn: DatabaseKey.userState),
                    let u_city = get.string(forColumn: DatabaseKey.userCity),
                    let u_gender = get.string(forColumn: DatabaseKey.userGender)
                {
                    userDefaultsDataSave(strid: u_id, strfirstname: u_firstname, strlastname: u_lastname, stremail: u_email, strpass: u_password, strmobileNo: u_mobileno, straboutme: u_aboutme,strImage: u_image,strDateofBirth: u_dateofbirth,strBirthTime: u_birthtime,strCountry: u_country,strState: u_state,strCity: u_city,strGender: u_gender)
                    return true
                }else{
